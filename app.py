...
def initialize() -> None:
    defaults = {"session_token": str(uuid.uuid4()), "language": "pt", "authenticated": False, "guest_name": "", "guest_phone": "", "guest_token": "", "selected_experience": None, "step_index": 0, "saved_answers": set(), "celebrated": False}
...
def render_login() -> None:
    ...
    st.markdown(f'<section class="yv-cinema"><div class="yv-cinema-bg" style="{background}"></div><div class="yv-orb"></div><div class="yv-cinema-content"><div class="yv-kicker">{esc(tr("welcome"))}</div><div class="yv-h1">{esc(tr("app_title"))}</div><div class="yv-story">{esc(opening)}</div><br><div class="yv-white-muted">{esc(tr("identify"))}</div></div></section>', unsafe_allow_html=True)
    st.markdown('<div class="yv-card">', unsafe_allow_html=True)
    first, second, third = st.columns([1.2, 1, .7])
    with first:
    ...
    if st.button(tr("start"), use_container_width=True):
...
def render_landing() -> None:
    ...
    for _, series in data.iterrows():
        ...
        st.markdown(f'<div class="yv-card"><div class="yv-kicker">{esc(tr("available"))}</div><div class="yv-h2">{esc(localized(row, "nome_sessao", tr("app_title")))}</div><div class="yv-muted"><b>{esc(localized(row, "subtitulo"))}</b><br>{esc(localized(row, "descricao_card"))}</div>{overview(experience_id)}</div>', unsafe_allow_html=True)
...
def render_journey(row: Dict[str, Any], position: int, total: int) -> None:
    ...
    st.markdown(f'<section class="yv-cinema"><div class="yv-cinema-bg" style="{background}"></div><div class="yv-orb"></div><div class="yv-cinema-content"><div class="yv-kicker">{esc(localized(row, "conceito_sensorial"))}</div><div class="yv-h1">{esc(localized(row, "titulo_tela"))}</div><div class="yv-story"><b>{esc(localized(row, "subtitulo_tela"))}</b></div><div class="yv-blocks">{blocks}</div>{tiles}<br><div class="yv-white-muted">{esc(localized(row, "instrucao_cliente"))}</div></div></section>', unsafe_allow_html=True)
