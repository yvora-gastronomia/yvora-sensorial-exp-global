...
def first_available_experience() -> Dict[str, Any]:
    """
    Retorna a primeira experiência ativa para preencher a abertura
    da tela de identificação. Assim, título, texto e botão permanecem
    administráveis pela planilha e respeitam o idioma selecionado.
    """
...
def available_journeys(experience_id: str) -> pd.DataFrame:
    ...
    data = data[
        data["experience_id"].astype(str).str.strip() == experience_id
    ].copy()

    if "ativo" in data.columns:
        data = data[data["ativo"].apply(active)]

    data["ordem"] = pd.to_numeric(
        data.get("ordem", 0),
        errors="coerce",
    ).fillna(0).astype(int)
...
def render_login() -> None:
    ...
    background = (
        f"background-image:url('{esc(image)}');"
        if image
    ...
    st.markdown(
        ...
        <section class="yv-cinema">
            <div class="yv-cinema-bg" style="{background}"></div>
            <div class="yv-orb"></div>
            <div class="yv-cinema-content">
                <div class="yv-kicker">{esc(tr("welcome"))}</div>
                <div class="yv-h1">{esc(title)}</div>
                <div class="yv-story">{esc(opening)}</div>
            </div>
        </section>
        """,
...
def overview(experience_id: str) -> str:
    ...
    if "tipo_tela" in data.columns:
        visible = data[
            data["tipo_tela"].astype(str).str.strip().str.lower() == "jornada"
        ]
    ...
    for _, series in data.iterrows():
        ...
        cards.append(
            f"""
            <div class="yv-overview-card">
                <b>
                    {esc(tr("journey"))} {esc(row.get("jornada_numero"))}
                    - {esc(localized(row, "conceito_sensorial"))}
                </b>
                <br>
                {esc(localized(row, "instrucao_cliente"))}
            </div>
            """
        )
...
def render_landing() -> None:
    ...
    for _, series in data.iterrows():
        ...
        st.markdown(
            f"""
            <div class="yv-card">
                <div class="yv-kicker">{esc(tr("available"))}</div>
                <div class="yv-h2">
                    {esc(localized(row, "nome_sessao", APP_TITLE))}
                </div>
                <div class="yv-muted">
                    <b>{esc(localized(row, "subtitulo"))}</b>
                    <br>
                    {esc(localized(row, "descricao_card"))}
                </div>
                {overview(experience_id)}
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if st.button(
            localized(row, "botao_inicio", tr("start")),
            key="start_" + experience_id,
...
def render_journey(row: Dict[str, Any], position: int, total: int) -> None:
    ...
    blocks = "".join(
        ...
        <div class="yv-story-block">
            <div class="yv-story-block-title">{esc(title)}</div>
            <div>{esc(body)}</div>
        </div>
        """
        for title, body in story_blocks(localized(row, "texto_principal"))
    )
    ...
    <div class="yv-steps">
        <div class="yv-step">
            <b>1. {esc(tr("meat"))}</b>
            <br>
            {esc(localized(row, "carne"))}
        </div>
        <div class="yv-step">
            <b>2. {esc(tr("cheese"))}</b>
            <br>
            {esc(localized(row, "queijo"))}
        </div>
        <div class="yv-step">
            <b>3. {esc(tr("together"))}</b>
            <br>
            {esc(tr("together_note"))}
        </div>
        <div class="yv-step">
            <b>4. {esc(beverage_label(row.get("experience_id")))}</b>
            <br>
            {esc(localized(row, "vinho"))}
        </div>
    </div>
    """
    ...
    st.markdown(
        ...
        <section class="yv-cinema">
            ...
            <div class="yv-cinema-content">
                ...
                <div class="yv-h1">
                    {esc(localized(row, "titulo_tela"))}
                </div>
                <div class="yv-story">
                    <b>{esc(localized(row, "subtitulo_tela"))}</b>
                </div>
                <div class="yv-blocks">{blocks}</div>
                {tiles}
                <br>
...
def store_feedback(row: Dict[str, Any], answer: str, comment: str = "") -> None:
    ...
    write_row(
        "feedbacks",
        {
            ...
            "nome_jornada": localized(row, "titulo_tela"),
    ...
    st.markdown(
        ...
        <section class="yv-cinema">
            <div class="yv-cinema-content yv-celebration">
                ...
                <div class="yv-h1">{esc(tr("completed"))}</div>
                <div class="yv-story">{esc(tr("thanks"))}</div>
                <br>
                <div class="yv-white-muted">{esc(tr("closing"))}</div>
            </div>
        </section>
        """,
        unsafe_allow_html=True,
...
def render_experience(experience_id: str) -> None:
    ...
    answer = st.radio(
        ...
        horizontal=True,
        index=None,
        key="answer_" + safe(row.get("etapa_id")),
    )
    ...
    with forward:
        if st.button(
            localized(row, "botao_texto", tr("next")),
            use_container_width=True,
        ):
